...
(function () {
  ...
  function loadPageData(page) {
    if (page === 'home') { loadHomeGallery(); }
    if (page === 'about') { loadAbout(); loadTeam(); }
    if (page === 'services') { loadServices(); }
    if (page === 'training') { loadWorkshops(); }
    if (page === 'work') { loadWorkGallery(); }
  }
  ...
  function createGalleryItem(p, i, arr) {
    ...
    div.className = 'gallery-item' + (i === 0 && arr.length > 3 ? ' featured' : '');
    div.dataset.index = i;
